...
export const useToDoStore = create<ToDoStore>((set, get) => ({
    ...
    addList: (name: string, examDate: Date) => set((state) => ({
        modules: [...state.modules, new Todolist("klsdfjsdkflj", name, [], examDate)],
    })),
    removeList: (listId: string) => set((state) => ({modules: state.modules.filter((list) => list.id !== listId)})),
    updateList: (updatedList: Todolist) => set((state) => ({
        modules: state.modules.map((list) => (list.id === updatedList.id ? updatedList : list)),
    })),
    ...
    renameModule: (listId: string, newName: string) => set((state) => ({
        modules: state.modules.map((list) => list.id === listId ? {...list, name: newName} : list)
    })),
}));